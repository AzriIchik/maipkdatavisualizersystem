...
var Notify = function (options) {
  ...
  var build = function () {
    if (jQuery('.notify-js.notify-wraper').length) { return }
    ...
    if (!isMobile()) { jQuery('body').append('<div class="notify-js notify-wraper notify-' + (isMobile() ? 'mobile' : 'desktop notify-left') + '"></div>') }
  }

  var init = function (options) {
    if (typeof options === 'string') { _this.options.content = options } else if (typeof options === 'object') { _this.options = Object.assign(_this.options, options) } else { console.error(_this.name + ': Argument error, string or object are accepted.') }
