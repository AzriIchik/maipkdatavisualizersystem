;(function ($) {
  ...
  var sys = {
    ...
    template: '<div id="{{id}}" class=" fadeIn el-notify-box {{className}} {{direction}}">' +
            　　　　　'{{iconHTML}}' +
                    '<div class="{{className}}-group" style="bottom:0px">' +
  ...
  $.notify.close = function (id, options) {
    ...
    for (var i = options.index + 1; i <= options.len; i++) {
      ...
      $item.animate({
        bottom: bottom - options.height - options.offset
      }, 'normal')
    }
  }
